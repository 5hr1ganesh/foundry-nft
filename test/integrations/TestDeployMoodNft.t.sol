...
contract TestDeployMoodNft is Test {
    ...
    function testConvertSvgToImageUri() public view {
        ...
        string
            memory expectedSvg = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="500" height="500"><text x="0" y="15" fill="black">Hi! Your browser decoded this</text></svg>';

        string memory actualUri = deployer.svgToImageURI(expectedSvg);
        assert(
    ...
    function testrun() public view {
        ...
        string memory sadSVG = vm.readFile(sadSvgPath);
        string memory happySVG = vm.readFile(happySvgPath);

        console2.log(sadSVG);
        console2.log(happySVG);

        assert(keccak256(abi.encodePacked(sadSVG)).length > 0);
        assert(keccak256(abi.encodePacked(happySVG)).length > 0);
    }
}
